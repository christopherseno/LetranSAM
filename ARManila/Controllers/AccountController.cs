        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    //ApplicationUser user = UserManager.FindById(UserManager.Users.FirstOrDefault().Id);
                    ...
                    if (aspnetuser != null && (aspnetuser.BearerToken == null || aspnetuser.BearerToken.Length < 1 || aspnetuser.TokenExpiration.HasValue == false || aspnetuser.TokenExpiration.Value < DateTime.Now))
                    {
                            ...
                            var tokenRequest = new List<KeyValuePair<string, string>>
                        {
                            ...
                            HttpResponseMessage response = httpClient.PostAsync("https://api.letran.edu.ph/Token", encodedRequest).Result;
                            token = response.Content.ReadAsAsync<BearerToken>().Result;

                            // Store token in ASP.NET Session State for later use
                            //HttpCookie cookie = new HttpCookie("ApiAccessToken", token.AccessToken);
                            //cookie.Expires.AddHours(8);
                            //cookie.Secure = true;
                            //HttpContext.Response.Cookies.Add(cookie);
                            //Session["ApiAccessToken"] = token.AccessToken;

                            aspnetuser.BearerToken = token.AccessToken;
                            aspnetuser.TokenExpiration = Convert.ToDateTime(token.Expires);
                            db.SaveChanges();
