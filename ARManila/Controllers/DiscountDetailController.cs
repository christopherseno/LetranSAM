...
{
    // Student discounts, summarized per academic department and fee from
    // [AR].[ArTrailDiscountDetail2024], with the same Generate -> Draft -> Post (Finalize)
    // workflow and month N-of-M deferral as MemoAdjustmentController / DeferredIncomeController.
    //
    ...
    // Persisted via raw SQL to AR.DiscountPosting / AR.DiscountPostingDetail
    // (run Database/DiscountPosting.Tables.sql once). The display DTOs are shared with the
    // memo/adjustment feature; discount has a single "Discount" section.
    public class DiscountDetailController : BaseController
    {
        private LetranIntegratedSystemEntities db = new LetranIntegratedSystemEntities();
        private Employee employee;
        protected Period Period { get; private set; }

        // Discount has a single logical type; kept as an ordered list for parity with the memo feature.
        private static readonly Tuple<string, string>[] DiscTypeOrder =
        {
        ...
        public ActionResult Index()
        {
            ...
            var batches = db.GetDiscountPostingBatches(Period.PeriodID);
        ...
        public ActionResult Save(DateTime asofdate, int nthmonth, int noofmonths)
        {
            ...
            int result = db.SaveDiscountPosting(Period.PeriodID, postingdate, employee.EmployeeID, nthmonth, noofmonths, entries);
            if (result == 0)
            {
        ...
        public ActionResult Finalize(string date)
        {
            ...
            int affected = db.FinalizeDiscountPosting(Period.PeriodID, postingdate);
            if (affected == 0)
            {
        ...
        public ActionResult Delete(string date)
        {
            ...
            postingdate = postingdate.Date;
            int result = db.DeleteDiscountPosting(Period.PeriodID, postingdate);
            if (result == -1)
            {
            ...
            {
                TempData["Message"] = "Deleted discount posting for " + postingdate.ToShortDateString() + ".";
            }
            return RedirectToAction("Index");
        }
        ...
        public ActionResult Details(string date)
        {
            ...
            if (model == null)
            {
                ViewBag.Error = "No saved discount posting found for " + postingdate.ToShortDateString() + ".";
                return View((MemoAdjustmentQueryDTO)null);
            }
            return View(model);
        }
        ...
        public ActionResult Excel(string date)
        {
            ...
            MemoAdjustmentQueryDTO model = BuildFromSaved(postingdate);
            if (model == null) { return Content("No saved discount posting found for " + postingdate.ToShortDateString() + "."); }

            byte[] bytes = ReportExcel.MemoMatrix(model, "Student Discounts");
            return File(bytes, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                "Discounts_" + postingdate.ToString("dd-MMMM-yyyy") + ".xlsx");
        }
        ...
        public ActionResult Print(string date)
        {
            ...
            if (model == null)
            {
                return Content("No saved discount posting found for " + postingdate.ToShortDateString() + ".");
            }
            ...
            {
                FileName = "Discounts_" + postingdate.ToString("dd-MMMM-yyyy") + ".pdf",
        ...
        {
            List<ArTrailMemoDetailRow> rows = db.GetArTrailDiscountDetail(Period.PeriodID, asof.ToString("yyyy-MM-dd"));
                                  ...
                                  .GroupBy(r => new { r.MemoType, Dept = r.AcaDeptID.Value, r.FeeID }))
            {
                ...
                {
                    MemoType = string.IsNullOrEmpty(g.Key.MemoType) ? "Discount" : g.Key.MemoType,
        ...
        {
            var saved = db.GetSavedDiscountDetails(Period.PeriodID, postingdate);
            if (saved == null || saved.Count == 0)
            {
                ...
                var hdr = db.GetDiscountPostingHeader(Period.PeriodID, postingdate);
        ...
        private MemoGLDTO BuildGL(DateTime postingdate)
        {
            ...
            foreach (var disc in DiscTypeOrder)
            {
